...
async function guardarCliente(to, msg) {
  ...
  try {
    ...
    const client = twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
    await client.messages.create({ from: TWILIO_NUMBER, to: to, body: String.fromCodePoint(0x1F465) + ' Cliente guardado en base de datos:\n' + cliente.nombre + (cliente.empresa ? ' - ' + cliente.empresa : '') + (cliente.telefono ? '\nTel: ' + cliente.telefono : '') + (cliente.notas ? '\nNotas: ' + cliente.notas : '') });
...
async function listarClientes(to) {
  ...
  try {
    ...
    lista += 'Para borrar: "borrar cliente [numero]"';
...
async function borrarCliente(to, msg) {
  ...
  try {
    ...
    if (num < 0 || num >= res.rows.length) {
      await client.messages.create({ from: TWILIO_NUMBER, to: to, body: 'Numero invalido. Usa "clientes" para ver la lista.' });
      return;
    }
...
async function programarRecordatorio(to, msg) {
  ...
  try {
    ...
    const client = twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
    await client.messages.create({ from: TWILIO_NUMBER, to: to, body: String.fromCodePoint(0x2705) + ' Agendado en base de datos:\n' + reminder.texto + '\n' + String.fromCodePoint(0x1F4C5) + ' ' + reminder.descripcionHumana });
...
async function chat(to, mensaje) {
  ...
  try {
    ...
    const systemPrompt = 'Sos un asistente personal de mesa de dinero argentina llamado EduBot. Tenes una base de datos PostgreSQL propia donde se guardan clientes, recordatorios y dailies. Los datos que ves abajo son de tu base de datos y persisten entre sesiones. NUNCA digas que no tenes memoria - SI tenes base de datos propia. Conoces el mercado local: Rofex, bonos, Lecaps, cauciones, dolar, BCRA. Busca en la web para info actual. Respondas directo y preciso. Maximo 5 lineas salvo que pidan algo largo.' + (dailyMemory[to] ? '\n\nInforme de hoy: ' + dailyMemory[to] : '') + clientesStr + recordatoriosStr;
